"""Formulation designer - the core orchestration of the deterministic layer.

Takes user inputs (drug, components + mol%, solvent, carrier, route, family,
process, pH, temperature, total membrane umol) and returns:
    * a descriptor row
    * heuristic CQA estimates (each with a numeric sub-score and drivers)
    * a batch-mass table
    * a design maturity level (DML-0..4)
    * an executive decision card

All CQA numbers are descriptor-driven decision-support estimates. They are
heuristic until trained/validated on internal experimental outcomes and require
...
class DesignInput:
    ...
    # list of (material_name_or_id, role, mol_percent)
...
class CQA:
    key: str
    label: str
    estimate: str          # qualitative estimate ("high", "small", "~68%", ...)
...
def design(inp: DesignInput, db: Optional[Database] = None) -> DesignResult:
    ...
    desc = compute_descriptors(
        comps, drug=drug_card, drug_mol_percent=inp.drug_mol_percent,
        solvent=solvent_card, pH=inp.pH, temperature_C=inp.temperature_C,
    )
    ...
    return DesignResult(
        inp=inp, components=comps, drug_card=drug_card, solvent_card=solvent_card,
        descriptors=desc, cqas=cqas, batch_table=batch, maturity_level=level,
        maturity_label=label, executive_decision=decision, nanoform_score=nano,
...
def _estimate_cqas(inp, comps, drug, solvent, desc, db) -> List[CQA]:
    ...
    if isinstance(logp, (int, float)):
        if logp >= 2:
            ee_score += 0.2; ee_dr.append(f"Lipophilic drug (logP {logp:.1f}) partitions into carrier")
        elif logp < 0:
            ee_score -= 0.2; ee_rk.append(f"Hydrophilic drug (logP {logp:.1f}); low passive lipid entrapment")
    red = v.get("drug_bilayer_red")
    if isinstance(red, (int, float)):
        if red < 1:
            ee_score += 0.15; ee_dr.append(f"Drug-bilayer RED {red:.2f} < 1 (compatible)")
        else:
            ee_score -= 0.1; ee_rk.append(f"Drug-bilayer RED {red:.2f} > 1 (limited compatibility)")
    ...
    cqas.append(CQA("encapsulation_efficiency", "Encapsulation efficiency",
                    _band(ee_score, ("low", "moderate", "high")), ee_score, ee_dr, ee_rk))
    ...
    if any(k in proc for k in ("microfluid", "high-pressure", "homogen", "sonicat", "extrus")):
        ps_score += 0.2; ps_dr.append("High-energy process favors smaller particles")
    if any(k in proc for k in ("hydration", "hand-shaken", "vortex")):
        ps_score -= 0.15; ps_rk.append("Low-energy hydration tends to larger vesicles (needs size reduction)")
    ps_score += 0.2 * high_hlb + 0.15 * peg
    if peg > 0:
        ps_dr.append("PEGylation adds steric stabilization")
    ps_score = eq.clamp(ps_score)
    cqas.append(CQA("particle_size", "Particle size tendency",
                    _band(ps_score, ("large", "moderate", "small")), ps_score, ps_dr, ps_rk))

    # --- PDI --------------------------------------------------------------- #
    pdi_score, pdi_dr, pdi_rk = 0.55, [], []
    n_struct = len([c for c in comps if c.role not in ("drug",)])
    if n_struct <= 3:
        pdi_score += 0.15; pdi_dr.append("Few components -> simpler self-assembly")
    if n_struct >= 5:
        pdi_score -= 0.15; pdi_rk.append("Many components can broaden the distribution")
    if high_hlb + peg > 0.05:
        pdi_score += 0.1; pdi_dr.append("Adequate stabilizer present")
    pdi_score = eq.clamp(pdi_score)
    cqas.append(CQA("pdi", "PDI (uniformity) tendency",
                    _band(pdi_score, ("broad", "moderate", "narrow")), pdi_score, pdi_dr, pdi_rk))
    ...
    if charged > 0.02:
        zt_score += 0.4; zt_dr.append(
            f"{'Anionic' if sign < 0 else 'Cationic' if sign > 0 else 'Charged'} inducer "
            f"({charged*100:.0f} mol%) raises |zeta|")
    else:
    ...
    if sign > 0:
        zt_rk.append("Cationic surface: uptake benefit but higher cytotoxicity/opsonization risk")
    zt_score = eq.clamp(zt_score)
    cqas.append(CQA("zeta_potential", "Zeta-potential tendency",
                    _zeta_label(zt_score, sign), zt_score, zt_dr, zt_rk))
    ...
    cqas.append(CQA("drug_loading", "Drug loading",
                    f"{dl:.1f} mol% of membrane", dl_score, dl_dr, dl_rk))

    # --- Release tendency -------------------------------------------------- #
    rel_score, rel_dr, rel_rk = 0.5, [], []
    if isinstance(rig, (int, float)):
        if rig > 0.6:
            rel_score += 0.2; rel_dr.append("Rigid/ordered membrane -> slower, sustained release")
        elif rig < 0.4:
            rel_score -= 0.1; rel_rk.append("Fluid membrane -> faster release / burst risk")
    ...
    cqas.append(CQA("release_tendency", "Release control tendency",
                    _band(rel_score, ("burst-prone", "moderate", "sustained")), rel_score, rel_dr, rel_rk))

    # --- Crystallization risk (lipid systems) ------------------------------ #
    cr_score, cr_dr, cr_rk = 0.6, [], []  # higher score = LOWER risk (favorable)
    ...
    if inp.family == "nanostructured_lipid_carrier":
        if any(c.card.material_type == "liquid_lipid" for c in comps):
            cr_score += 0.2; cr_dr.append("Liquid-lipid domain disrupts crystallinity (NLC advantage)")
    highmelt = [c for c in comps if isinstance(c.card.get("melting_point_C"), (int, float))
                and c.card.get("melting_point_C") > 65]
    if highmelt:
        cr_rk.append("High-melting lipid(s) present: " + ", ".join(c.name for c in highmelt))
    cr_score = eq.clamp(cr_score)
    cqas.append(CQA("crystallization_risk", "Crystallization / expulsion risk",
                    _band(cr_score, ("high risk", "moderate", "low risk")), cr_score, cr_dr, cr_rk))

    # --- Solvent suitability ----------------------------------------------- #
    sv_score, sv_dr, sv_rk = 0.5, [], []
    sred = v.get("drug_solvent_red")
    if isinstance(sred, (int, float)):
        sv_score = eq.clamp(1.0 - sred)
        (sv_dr if sred < 1 else sv_rk).append(
            f"Drug-solvent RED {sred:.2f} ({'inside' if sred < 1 else 'outside'} solubility sphere)")
    ...
    cqas.append(CQA("solvent_suitability", "Solvent suitability",
                    _band(sv_score, ("poor", "moderate", "good")), sv_score, sv_dr, sv_rk))
    ...
    if inp.carrier and carrier_card is not None:
        ...
        if match:
            ...
            (ca_dr if ca_score >= 0.5 else ca_rk).append(
                f"{match['carrier']} rational-fit score {ca_score:.2f} for {inp.route}/{inp.process_method}")
            if match["warnings"]:
        ...
        ca_score = 0.55
    cqas.append(CQA("carrier_suitability", "Carrier / cryoprotectant suitability",
                    _band(ca_score, ("poor", "moderate", "good")), ca_score, ca_dr, ca_rk))

    # --- NanoForm composite score ------------------------------------------ #
    ...
    cqas.append(CQA("nanoform_score", "NanoForm composite score",
                    f"{nano:.2f} / 1.00", nano,
                    [f"Weighted by design goal '{inp.design_goal}'"], []))
    return cqas


def _goal_weights(goal: str) -> Dict[str, float]:
    base = {
        "encapsulation_efficiency": 0.2, "particle_size": 0.15, "pdi": 0.1,
        "zeta_potential": 0.1, "drug_loading": 0.1, "release_tendency": 0.1,
        "crystallization_risk": 0.1, "solvent_suitability": 0.1,
...
def _batch_table(inp, comps, drug) -> List[Dict[str, Any]]:
    ...
    for c in comps:
        ...
        rows.append({
            "component": c.name, "role": c.role, "mol_percent": c.mol_percent,
            "umol": round(umol, 3), "MW": mw,
            "mg": round(mg, 3) if mg is not None else None,
        })
    if drug is not None:
        ...
        mg = eq.mg_from_umol(umol, mw) if isinstance(mw, (int, float)) else None
        rows.append({
            "component": drug.name, "role": "drug", "mol_percent": inp.drug_mol_percent,
            "umol": round(umol, 3), "MW": mw,
            "mg": round(mg, 3) if mg is not None else None,
        })
    return rows
...
def _maturity(desc, comps, missing) -> Tuple[int, str]:
    if not comps:
        return 0, MATURITY[0]
    # No internal outcome data exist in this build -> cap at DML-2.
